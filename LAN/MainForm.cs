...
namespace Messenger
{
    public partial class MainForm : Form
    {
        ...
        public MainForm()
        {
            ...
            _profileDirectory = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "LAN Messenger"); // Đường dẫn đến thư mục ApplicationData cho ứng dụng
            _userNameFilePath = Path.Combine(_profileDirectory, "user_profile.ini"); // Đường dẫn đến tệp tin lưu trữ tên người dùng
            _chatHistoryDirectory = Path.Combine(Path.GetDirectoryName(Application.ExecutablePath), "LOG"); // Đường dẫn đến thư mục "LOG" cùng cấp với tệp thực thi để lưu lịch sử chat
        ...
        private void InitializeSystemTray()
        {
            ...
            {
                Text = "LAN Messenger", // Văn bản hiển thị khi di chuột qua biểu tượng
            ...
            if (WindowState == FormWindowState.Minimized && !_isClosing)
            {
                ...
                if (_trayIcon.Icon != null)
                {
                    ...
                    balloon.ShowFromBottom(
                        "LAN Messenger", // Tiêu đề thông báo
        ...
        protected override void OnFormClosing(FormClosingEventArgs e)
        {
            ...
            {
                Debug.WriteLine($"[OnFormClosing] Lỗi khi lưu lịch sử: {ex.Message}");
            }
        }
                ...
                if (renameForm.ShowDialog(this) == DialogResult.OK)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(newName) && newName != _myUserName)
                    {
                        ...
                        try
                        {
                            ...
                            this.Text = $"LAN Messenger - {_myUserName}"; // Cập nhật tiêu đề form
        ...
        private void ChatListBox_MeasureItem(object sender, MeasureItemEventArgs e)
        {
            ...
            try
            {
                ...
                e.ItemHeight = (int)message.CalculatedTotalSize.Height + 5; // Giảm padding từ 20 xuống 5 để giảm khoảng cách
        ...
        private void ChatListBox_DrawItem(object sender, DrawItemEventArgs e)
        {
            ...
            try
            {
                ...
                Debug.WriteLine($"Vẽ tin nhắn tại index {e.Index}, Bounds.Y = {e.Bounds.Y}, Height = {e.Bounds.Height}, Visible = {e.Bounds.IntersectsWith(chatListBox.ClientRectangle)}");
            }
            catch (Exception ex)
            {
